...
if __name__ == "__main__":
    ...
    if not (
        ...
        print("Save history..")
        save(
            f"{temp_data_folder}/browser_history_private.json", filtered_history_private
        )
        save(f"{temp_data_folder}/browser_history_public.json", filtered_history_public)
        save(
            f"{temp_data_folder}/scheme_stats.json",
            filtered_history_public,
            mode="scheme",
        )
    else:
    ...
    history1 = Path(
        f"{temp_data_folder}/browser_history_private.json",
    )
    history2 = Path(
        f"{temp_data_folder}/browser_history_public.json",
    )
    ...
    # Save
    df_results.to_csv(
        f"{temp_data_folder}/similarity_private_public_results.csv", index=False
    )
